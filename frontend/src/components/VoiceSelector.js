...
export default function VoiceSelector({ voices = [], selectedId, onSelect }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
      {voices.map((v, idx) => {
        ...
        return (
          <motion.button type="button" onClick={() => onSelect && onSelect(vid)} key={vid} whileHover={{ y: -3 }} className={`text-left card p-3 flex items-center gap-3 ${active ? "ring-2 ring-[color:var(--color-primary)]" : ""}`}>
